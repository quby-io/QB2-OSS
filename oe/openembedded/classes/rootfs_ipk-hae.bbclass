...
fakeroot rootfs_ipk_do_rootfs () {
	...
	# first mark all as 'installed'
	for i in ${IMAGE_ROOTFS}${libdir}/opkg/info/*.control; do
		opkg-cl ${IPKG_ARGS} flag installed `basename $i .control`
	done

	# then one-by-one possibly as 'unpacked' if the scripts fail
